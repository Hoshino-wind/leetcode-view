...
function PlusOneVisualizer() {
  const visualization = useVisualization<PlusOneInput>(
    (input) => generatePlusOneSteps(input.digits),
    { digits: [1, 2, 9] }
  );

  const data = visualization.currentStepData?.data as { digits?: number[] } || {};
  const { digits = [] } = data;
  const variables = visualization.currentStepData?.variables;
  const carry = getNumberVariable(variables, 'carry');
  const index = getNumberVariable(variables, 'index');
  const hasCarry = getBooleanVariable(variables, 'hasCarry');
  const noCarry = getBooleanVariable(variables, 'noCarry');
  const overflow = getBooleanVariable(variables, 'overflow');
  ...
  const customVariables = (variables: StepVariables) => {
    const carry = getNumberVariable(variables, 'carry');
    const index = getNumberVariable(variables, 'index');
    const currentDigit = getNumberVariable(variables, 'currentDigit');
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
        ...
        {carry !== undefined && (
          <div>
            ...
            <span className="text-gray-500"> = </span>
            <span className="font-mono text-gray-800 font-semibold">{carry}</span>
          </div>
        )}
        {currentDigit !== undefined && (
          <div>
            ...
            <span className="text-gray-500"> = </span>
            <span className="font-mono text-gray-800 font-semibold">{currentDigit}</span>
          </div>
        )}
        {sum !== undefined && (
          <div>
  ...
  return (
    <VisualizationLayout
      ...
      inputFields={[{ type: "array", key: "digits", label: "数字数组 digits", placeholder: "输入数字，如: 1,2,9" }]}
      testCases={[
    ...
    >
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="flex gap-2 items-center justify-center mb-6">
          {(digits as number[]).map((digit: number, idx: number) => {
