...
interface PalindromeNumberInput {
  x: number;
}

function PalindromeNumberVisualizer() {
  const visualization = useVisualization<PalindromeNumberInput>(
    (input) => generatePalindromeNumberSteps(input.x),
    { x: 12321 }
  );

  const variables = visualization.currentStepData?.variables;
  const original = getNumberVariable(variables, 'original');
  const digits = getArrayVariable(variables, 'digits') as number[] | undefined;
  const reversed = getNumberVariable(variables, 'reversed');
  const temp = getNumberVariable(variables, 'temp');
  const isPalindrome = getBooleanVariable(variables, 'isPalindrome');
  ...
  const customVariables = (variables: StepVariables) => {
    const temp = getNumberVariable(variables, 'temp');
    const reversed = getNumberVariable(variables, 'reversed');
    const digit = getNumberVariable(variables, 'digit');
    return (
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        {digits && digits.length > 0 ? (
          <div className="flex justify-center items-center gap-2 mb-6">
            {(digits as number[]).map((digit, index) => (
              <motion.div
                key={index}
