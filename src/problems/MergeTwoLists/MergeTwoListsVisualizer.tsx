...
function MergeTwoListsVisualizer() {
  const visualization = useVisualization<MergeTwoListsInput>(
    (input) => generateMergeTwoListsSteps(input.list1, input.list2),
    { list1: [1, 2, 4], list2: [1, 3, 4] }
  );

  const data = visualization.currentStepData?.data as { list1?: number[]; list2?: number[]; merged?: number[] } || {};
  const { list1 = [], list2 = [], merged = [] } = data;
  const variables = visualization.currentStepData?.variables;
  const pointer1 = getNumberVariable(variables, 'pointer1');
  const pointer2 = getNumberVariable(variables, 'pointer2');
  const comparing = getBooleanVariable(variables, 'comparing');
