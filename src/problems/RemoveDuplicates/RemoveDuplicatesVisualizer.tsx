...
function RemoveDuplicatesVisualizer() {
  const visualization = useVisualization<RemoveDuplicatesInput>(
    (input) => generateRemoveDuplicatesSteps(input.nums),
    { nums: [0, 0, 1, 1, 1, 2, 2, 3, 3, 4] }
  );

  const data = visualization.currentStepData?.data as { nums?: number[] } || {};
  const { nums = [] } = data;
  const variables = visualization.currentStepData?.variables;
  const k = getNumberVariable(variables, 'k');
  const i = getNumberVariable(variables, 'i');
  const moved = getBooleanVariable(variables, 'moved');
  const skipped = getBooleanVariable(variables, 'skipped');
  const finished = getBooleanVariable(variables, 'finished');
  const uniqueArray = getArrayVariable(variables, 'uniqueArray') as number[] | undefined;
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        {k !== undefined && (
          <div>
            ...
            <span className="font-mono text-gray-800 font-semibold">{k}</span>
          </div>
        )}
        {i !== undefined && (
          <div>
            ...
            <span className="text-gray-500"> = </span>
            <span className="font-mono text-gray-800 font-semibold">{i}</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <VisualizationLayout
      ...
      inputFields={[{ type: "array", key: "nums", label: "有序数组 nums", placeholder: "输入有序数组，如: 0,0,1,1,1,2,2,3,3,4" }]}
      testCases={[
    ...
    >
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="flex gap-2 items-center flex-wrap justify-center">
          {(nums as number[]).map((num: number, index: number) => {
