...
interface ReverseStringInput {
  s: string;
}

function ReverseStringVisualizer() {
  const visualization = useVisualization<ReverseStringInput>(
    (input) => generateReverseStringSteps(input.s),
    { s: "hello" }
  );

  const data = visualization.currentStepData?.data as { chars?: string[] } || {};
  const { chars = [] } = data;
  ...
  const swapped = getBooleanVariable(variables, 'swapped');
