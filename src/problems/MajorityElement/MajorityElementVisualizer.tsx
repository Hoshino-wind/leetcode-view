...
interface MajorityElementInput {
  nums: number[];
}

function MajorityElementVisualizer() {
  const visualization = useVisualization<MajorityElementInput>(
    (input) => generateMajorityElementSteps(input.nums),
    { nums: [3, 2, 3] }
  );
  ...
  const candidate = getNumberVariable(variables, 'candidate');
  const count = getNumberVariable(variables, 'count');
  const index = getNumberVariable(variables, 'index');
  const selectCandidate = getBooleanVariable(variables, 'selectCandidate');
  const voteFor = getBooleanVariable(variables, 'voteFor');
  const voteAgainst = getBooleanVariable(variables, 'voteAgainst');
