...
interface PascalTriangleInput {
  numRows: number;
}

function PascalTriangleVisualizer() {
  const visualization = useVisualization<PascalTriangleInput>(
    (input) => generatePascalTriangleSteps(input.numRows),
    { numRows: 5 }
  );

  const data = visualization.currentStepData?.data as { triangle?: number[][]; currentRow?: number[] } || {};
  const { triangle = [], currentRow = [] } = data;
  const variables = visualization.currentStepData?.variables;
  const currentRowIndex = getNumberVariable(variables, 'currentRow');
  const col = getNumberVariable(variables, 'col');
  const finished = getBooleanVariable(variables, 'finished');
  const isBorder = getBooleanVariable(variables, 'isBorder');

  const customVariables = (variables: StepVariables) => {
    const currentRow = getNumberVariable(variables, 'currentRow');
    const col = getNumberVariable(variables, 'col');
    const value = getNumberVariable(variables, 'value');
    const leftParent = getNumberVariable(variables, 'leftParent');
    const rightParent = getNumberVariable(variables, 'rightParent');
