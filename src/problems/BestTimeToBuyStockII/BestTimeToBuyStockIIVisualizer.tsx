...
interface BestTimeToBuyStockIIInput {
  prices: number[];
}

function BestTimeToBuyStockIIVisualizer() {
  const visualization = useVisualization<BestTimeToBuyStockIIInput>(
    (input) => generateBestTimeToBuyStockIISteps(input.prices),
    { prices: [7, 1, 5, 3, 6, 4] }
  );

  const data = visualization.currentStepData?.data as { prices?: number[] } || {};
  const { prices = [] } = data;
  const variables = visualization.currentStepData?.variables;
  const day = getNumberVariable(variables, 'day');
  const totalProfit = getNumberVariable(variables, 'totalProfit');
  const profit = getNumberVariable(variables, 'profit');
  const buyDay = getNumberVariable(variables, 'buyDay');
  const sellDay = getNumberVariable(variables, 'sellDay');
  const hasProfit = getBooleanVariable(variables, 'hasProfit');
