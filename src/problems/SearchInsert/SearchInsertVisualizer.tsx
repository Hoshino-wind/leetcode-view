...
function SearchInsertVisualizer() {
  const visualization = useVisualization<SearchInsertInput>(
    (input) => generateSearchInsertSteps(input.nums, input.target),
    { nums: [1, 3, 5, 6], target: 5 }
  );

  const data = visualization.currentStepData?.data as { nums?: number[]; target?: number } || {};
  ...
  const mid = getNumberVariable(variables, 'mid');
  const found = getBooleanVariable(variables, 'found');
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
        ...
        {mid !== undefined && (
          <div>
            ...
            <span className="text-gray-500"> = </span>
            <span className="font-mono text-gray-800 font-semibold">{mid}</span>
          </div>
        )}
        {target !== undefined && (
          <div>
